...
def fetch_balances(address):
    url = f"https://warden-testnet-api.itrocket.net/cosmos/bank/v1beta1/balances/{address}"
...
def fetch_validators():
    url = "https://warden-testnet-api.itrocket.net/cosmos/staking/v1beta1/validators?pagination.limit=300&status=BOND_STATUS_BONDED"
    response = requests.get(url, verify=False)
    if response.status_code == 200:
...
def update_max_amount(entry, warden_address, root):
    current_balances = fetch_balances(warden_address)  # Her seferinde güncel bakiyeleri çek
    total_balance_wei = sum(int(balance['amount']) for balance in current_balances if balance['denom'] == 'uward')
    total_balance_uward = total_balance_wei / 1e6
    max_transferable_uward = max(0, total_balance_uward - 0.01)

    entry.delete(0, tk.END)
    if max_transferable_uward > 0:
        entry.insert(0, f"{max_transferable_uward:.6f}")
    else:
        entry.insert(0, "0.000000")
        messagebox.showinfo("Insufficient Balance", "You need at least 0.01 uward more in your wallet to perform transactions.", parent=root)


def update_balance(balance_label, address):
    balances = fetch_balances(address)
    if balances:
        balance_str = " ".join([f"{format_balance(balance['amount'])} WARD" if balance['denom'] == 'uward' else f"{format_balance(balance['amount'])} {balance['denom']}" for balance in balances])
        balance_label.config(text=f"Balances: {balance_str}")
    else:
        balance_label.config(text="Balances: 0")

def perform_transfer(entry, target_address_entry, fee_entry, gas_entry, warden_address, private_key, root, balance_label):
    # Cüzdan adresi ve private key'in doğruluğunu kontrol edin
    #print(f"Transfer işlemi için kullanılan adres: {warden_address}, Private Key: {private_key}")

    try:
        amount_uward = float(entry.get())  # Kullanıcıdan alınan miktarı uward cinsinden al
        amount_wei = int(amount_uward * 10**6)  # Wei'ye çevir
    except ValueError:
        messagebox.showerror("Error", "Invalid amount entered. Please enter a numeric value.", parent=root)
        return

    # Güncel bakiyeleri çek
    current_balances = fetch_balances(warden_address)
    total_balance_wei = sum(int(balance['amount']) for balance in current_balances if balance['denom'] == 'uward')
    ...
    if total_balance_wei - amount_wei < 0.01 * 10**6:
        messagebox.showwarning("Warning", "Insufficient balance after transfer. Minimum balance of 0.01 uward required.", parent=root)
        return

    # Tüm kontroller geçilirse, transfer işlemini doğru sıra ile parametreleri vererek gerçekleştir
    success = transfer_token(target_address_entry.get(), str(amount_wei), 'uward', fee_entry.get(), gas_entry.get(), private_key)
    print("sonuc",success)
    if success==True:
        time.sleep(5)
        update_balance(balance_label, warden_address)


def perform_delegate(valid_adr, entry, gas_entry, fee_entry, warden_address, private_key, root,balance_label):
    try:
        amount_uward = float(entry.get())  # Kullanıcıdan alınan miktarı uward cinsinden al
        amount_wei = int(amount_uward * 10**6)  # Wei'ye çevir
    except ValueError:
    ...
    current_balances = fetch_balances(warden_address)  # Güncel bakiyeleri çek
    total_balance_wei = sum(int(balance['amount']) for balance in current_balances if balance['denom']== 'uward')
    ...
    # Tüm kontroller geçilirse, transfer işlemini doğru sıra ile parametreleri vererek gerçekleştir
    success = delegate_to_validator(valid_adr, str(amount_wei), gas_entry.get(),fee_entry.get(),warden_address,private_key)
    print("sonuc",success)
    if success==True:
        time.sleep(5)
        update_balance(balance_label, warden_address)

def wallet_actions(root, warden_address, private_key):
    clear_content(root)
    style = ttk.Style()
    style.theme_use('clam')
    style.configure('TFrame', background='#140901')
    style.configure('TButton', background='#CAFF94', foreground='black', font=('Arial', 12))
    style.configure('TLabel', background='#140901', foreground='white', font=('Arial', 12))
    ...
    warden_frame = ttk.Frame(panel2, style='TFrame')
    warden_frame.pack(fill='x', pady=5)
    ttk.Label(warden_frame, text="Warden Address:", font=('Arial', 10,'bold','underline'),foreground='orange').pack(side='left', padx=(10, 5))
    warden_label = ttk.Label(warden_frame, text=warden_address, font=('Arial', 11, 'bold'))
    warden_label.pack(side='left')
    warden_copy_button = ttk.Button(warden_frame, text="Copy",style='Small.TButton', command=lambda: copy_to_clipboard(root, warden_address))
    warden_copy_button.pack(side='left', padx=5)
    ...
    tab_control.add(tab_transfer, text='Ward Transfer')
    ...
    setup_tab(tab_transfer, "Transfer", validators, warden_address, private_key,root)
    setup_tab(tab_delegate, "Delegate", validators, warden_address, private_key,root)
    ...
    update_balance_button = ttk.Button(frame, text="Update Balance", command=lambda: update_balance(balance_label, warden_address))
...
def setup_tab(tab, action_type, validators, warden_address, private_key, root):
    if action_type == "Transfer":
        current_balances = fetch_balances(warden_address)  # Mevcut bakiyeleri çek
        ...
        max_button = ttk.Button(tab, text="Max", command=lambda: update_max_amount(amount_entry_transfer, warden_address, root))
        ...
        fee_entry.insert(0, "5000")
        fee_entry.pack(pady=(0,10))

        # Transfer butonu
        transfer_button = ttk.Button(tab, text='Transfer', command=lambda: perform_transfer(amount_entry_transfer, target_address_entry, fee_entry, gas_entry, warden_address, private_key, root,balance_label))
        ...
        validator_combobox.bind('<<ComboboxSelected>>', lambda event: on_validator_selected(event, validators, validator_combobox, warden_address, private_key))

        ttk.Label(tab, text='Amount:').pack(pady=(10,0))
        amount_entry_delegate = ttk.Entry(tab, width=20)
        amount_entry_delegate.pack (pady=(0,10))
        
        max_button = ttk.Button(tab, text="Max", command=lambda: update_max_amount(amount_entry_delegate, warden_address, root))
        max_button.pack(pady=(5, 10))
        ...
        ttk.Label(tab, text='Fee (amount):').pack(pady=(10,0))
        fee_entry = ttk.Entry(tab, width=20)
        fee_entry.insert(0, "5000")  # Sabit değer olarak fee miktarı
        fee_entry.pack(pady=(0,10))

        delegate_button = ttk.Button(tab, text='Delegate', command=lambda: perform_delegate(valid_adr, amount_entry_delegate, gas_entry, fee_entry, warden_address, private_key, root,balance_label))
...
def on_validator_selected(event, validators, combobox, warden_address, private_key):
